...
- **Lot 1** — validation de l'approximation Monte Carlo (`information_gain_mc` vs `information_gain_exact`) : 1.1 (les deux modes `sample_y`/`sample_z`) et E1 (fidélité de politique) faits, voir tableau ci-dessus. Restent : E2 (coût en aval — rejouer le benchmark complet avec π̂_N), E3 (coût de calcul vs `|Z|`), 1.5 (figure `|Z|` vs nombre de concepts).
